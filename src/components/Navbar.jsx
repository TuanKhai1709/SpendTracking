  ...
  return (
    <nav className="bottom-nav">
      ...
      <NavLink to="/expenses" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
        ...
        <span className="nav-label">{t('expenses')}</span>
      </NavLink>
      <NavLink to="/income" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
        <svg className="nav-icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <polyline points="23 6 13.5 15.5 8.5 10.5 1 18" /><polyline points="17 6 23 6 23 12" />
        </svg>
        <span className="nav-label">{t('income')}</span>
      </NavLink>
      ...
      <button onClick={toggleLang} className="nav-item nav-lang">
        <span className="lang-badge">{lang === 'en' ? '🇻🇳' : '🇺🇸'}</span>
        <span className="nav-label">{lang === 'en' ? 'VI' : 'EN'}</span>
      </button>
    </nav>
  );
}
